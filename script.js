    ...
    snapshot.forEach(docSnap => {
      ...
      div.innerHTML += `
        <div>
          ...
          <div>상태: ${d.status}</div>
          <button class="chat-btn" onclick="startChat('${id}', '${d.sellerId}')">채팅</button>
          ${d.sellerId === auth.currentUser.uid && d.status==="판매중" ? `<button class="sold-btn" onclick="markAsSold('${id}')">거래 완료</button>` : ""}
        </div>
      `;
    });
  });
...
window.startChat = async (itemId, sellerId) => {
  ...
  window.location.href = `chat.html?roomId=${currentRoomId}&sellerId=${sellerId}&buyerId=${buyerId}`;
};
